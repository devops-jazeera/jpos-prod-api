...
//             await this.sync_salesLine(lineData.recordset, salesdata.createddatetime);
//           }
//         } catch (err) {
//           log.error(err);
//         }
...
//           item.redeempts
//         },${item.redeemptsamt},'${item.deliverytype}', '${item.customerref}', 4,4, 966, ${
//           item.netamount
//         }, 0,0,0,0,'OFFLINE', 0);`;
//         log.info(query);
//         queryData.push(query);
//       }
//       await SyncServiceHelper.BatchQuery(this.localDbConfig, queryData, log);
//     } catch (err) {
//       log.error(err);
//     }
//   }
//   async sync_salesLine(salesLineData: any[], date: string, queryData: any[] = []) {
//     try {
//       for (let line of salesLineData) {
//         if (line.INVENTTRANSID && line.INVENTTRANSID != "") {
//           let colorantdata: any = salesLineData.find(
//             (v: any) => v.ITEMID == "HSN-00001" && v.CONFIGID == line.INVENTTRANSID && v.SALESQTY == line.SALESQTY
//           );
//           if (colorantdata) {
...
//           let MultiLineDiscRanges = await SyncServiceHelper.ExecuteQuery(
//             this.localDbConfig,
//             MultiLineDiscRangesQuery,
//             log
//           );
//           line.applied_discounts.push({
...
//           let InstantDiscRanges = await SyncServiceHelper.ExecuteQuery(this.localDbConfig, InstantDiscRangesQuery, log);
//           line.applied_discounts.push({
...
//           line.applied_discounts
//         }', ${line.is_parent}, 'POSTED', '${line.INVENTTRANSID}', '${line.COLORANTPRICE}')
//         `;
//         log.info(query);
//         queryData.push(query);
//       }
...
//           let salesOrderData = await SyncServiceHelper.ExecuteQuery(this.localDbConfig, text, log);
...
//         }
//         let saleslinequery = `select id, colorantid   from salesline where salesid = '${trans.INVOICEID}' AND itemid = '${trans.ITEMID}' AND configid = '${trans.ConfigId}' AND inventsizeid = '${trans.InventSizeId}' AND batchno = '${trans.BATCHNO}' AND salesqty = ABS(${trans.QTY}) limit 1`;
//         let salesLineData = await SyncServiceHelper.ExecuteQuery(this.localDbConfig, saleslinequery, log);
...
//         }', ${transactionclosed}, 'OLD_POS_DATA', '${trans.saleslineid}', now(), '${trans.reservationid}');
//         `;
//         log.info(query);
